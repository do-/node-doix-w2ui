...
function w2uiQuery (from, options = {}) {
	...
	const {request} = this.job, {sort, search} = request

	for (const k of ['offset', 'limit']) options [k] = request [k]

	if (sort) options.order = sort.map (o => [o.field, o.direction === 'desc'])

	const query = this.model.createQuery (from, options)

	if (search) (request.searchLogic === 'OR' ? addOr : addAnd) (query.root, search)
